...
class Rook(Piece):
    ...
    def __init__(self, x, y):
        ...
        '''
    Takes in a State object, checks the possible moves the Rook can take
    and checks if the coordinate at that grid is walkable ( >0 )
    Then returns list possible action outcomes (x,y) from this state
    @Returns list of tuple of numbers (x,y)
    ...
    def actions(self, grid, rows, cols, other_piece_locations):
        ...
        for i in range(1, x_avail+1):
            ...
            elif grid[x-i][y] >= 0 and (x-i,y) in other_piece_locations:
        ...
        for i in range(1, y_avail+1):
            ...
            if grid[x][y+i] >= 0 and (x, y+i) not in other_piece_locations:
                #if the spot is movable -> no obstacle and no piece blocking
                acts.append((x,y+i))
            elif grid[x][y+i] >= 0 and (x, y+i) in other_piece_locations:
                #if spot if only movable, but is in other piece locations
                acts.append((x,y+i))
                break
            else:   
    ...
    def actions(self, grid, rows, cols, other_piece_locations):
        ...
        while x_avail - 1 >= 0 and y_avail - 1 >= 0:
            if grid[x_avail-1][y_avail-1] >= 0 and ((x_avail-1, y_avail-1) not in other_piece_locations):
                ...
                x_avail -= 1
                y_avail -= 1
            elif grid[x_avail-1][y_avail-1] >= 0 and ((x_avail-1, y_avail-1) in other_piece_locations):
...
class Board:
    ...
    def __init__(self, rows, cols, grid, pieces):
        ...
        self.piece_locations = set() # in (r,c) format
    ...
    def get_threat_pairs(self):
        ...
        for piece in self.init_pieces:
            ...
            for loc in threat_locs:
                #if location is occupied by another piece means it threatens it
...
class State:
    ...
    def get_random_successor(self):
        #selects action (a State) which leads to best value
        list_actions = self.actions()
        leng = len(list_actions)
        if leng > 0:
            random_choice = randint(0,leng-1)
            return list_actions[random_choice]
        else:
            return None
    ...
    def value(self):
        final_val = 0
        for pairs in self.board.threat_pairs.values():
            final_val += len(pairs)
    ...
    def is_goal(state, k):
        ...
        if state.num_pieces >= k and state.value_ == 0:
            return True
        else:
            return False
    ...
    #     print("Is goal? ", State.is_goal(self, self.k))
...
def search(rows, cols, grid, pieces, k):
    ...
    # initial_state.print_statistics()

    #get all actions, actions are a list of (state_value, state)
    ...
    current = None
    #limit searches to continue if cant find, terminate
    while current is None or not State.is_goal(current, required_remaining_limit):
        
        # limit = 100
        current = random_initial_state(all_actions)
        
        while True: #limit remove first
            if (not current is None) and State.is_goal(current, required_remaining_limit):
                break
            # limit -= 1
            neighbour = current.get_highest_valued_successor()
            # neighbour = None
            # if highest_successor is not None:
            #     neighbour = highest_successor
        ...
        # if State.is_goal(current, required_remaining_limit):
        #     # print("Reached goal")
...
# rows, cols, grid, pieces, k = parse('Local4.txt')

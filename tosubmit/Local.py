...
class King(Piece):
    ...
    def __init__(self, x, y, colour):
        super().__init__(x,y,self.name, colour)
...
class Rook(Piece):
    ...
    def actions(self, grid, rows, cols, other_piece_locations):
        ...
        for i in range(1, x_avail+1):
            ...
            acts.append((x-i,y))
            if (x-i, y) in other_piece_locations:
                break
        ...
        for i in range(1, y_avail+1):
            ...
            acts.append((x, y+i))
                #if the spot is movable -> no obstacle and no piece blocking
                if (x, y+i) not in other_piece_locations:
